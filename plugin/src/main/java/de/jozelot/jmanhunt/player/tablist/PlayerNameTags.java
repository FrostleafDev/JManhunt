...
public class PlayerNameTags {
    ...
    public void updateNameTag(ManhuntPlayer player) {
        ...
        String teamDisplayName = teamColor + plugin.getBootstrap().getLangManager().format("teams." + player.getTeam().name().toLowerCase(), null);

        String format = config.getTeamPrefix().getFormat();
        String[] parts = format.split("\\{player_name}");

        if (parts.length > 0) {
            String prefixRaw = parts[0].replace("{team}", teamDisplayName);
        ...
        if (parts.length > 1) {
            String suffixRaw = parts[1].replace("{team}", teamDisplayName);
